...
- **VP0-D7 (новое).** Честность приёмки: механический/fake-результат не
  засчитывается как финальный PASS. Введены статусы `PASS`, `PASS_MECHANISM`,
  `GATE_REAL`, `FAIL`. Итог VP-0 не печатает «11/11 PASS», пока крит. 3–5 не
  закрыты реально. Runner-recovery усилен до продолжения-до-успеха (не просто
  пометки INTERRUPTED).
